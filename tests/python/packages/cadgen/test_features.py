...
class FeatureTest(unittest.TestCase):
    ...
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="cadgen-feature-")
        self.env = mock.patch.dict(os.environ, {
            "CADGEN_CACHE_DIR": self.temp.name,
            "CADGEN_FEATURE_CACHE": "1",
            "CADGEN_OP_MEMO": "1",
            "CADGEN_OP_MEMO_DISK": "1",
        })
        self.env.start()
        trust = mock.patch.object(features, "_reuse_trusted", True)
        trust.start()
        self.addCleanup(trust.stop)
        self.addCleanup(self.temp.cleanup)
        self.addCleanup(self.env.stop)
        for name in features._stats:
            features._stats[name] = 0
    ...
    def test_kernel_free_public_import(self):
        command = "import sys; from cadgen import feature; assert 'build123d' not in sys.modules; assert 'OCP' not in sys.modules"
        subprocess.run([sys.executable, "-c", command], check=True,
                       env={**os.environ, "PYTHONPATH": str(Path(features.__file__).parents[1])}, timeout=20)

    def test_cold_warm_disabled_and_fresh_process_recipe_are_identical(self):
        with self._build():
            first = _plate()
            warm = _plate(width=30.0, holes=3)
            with mock.patch.dict(os.environ, {"CADGEN_FEATURE_CACHE": "0"}):
                disabled = _plate(30.0)
            features._code_cache.clear()
            features._code_bytes = 0
            disk = _plate()
        self.assertEqual(2, features._stats["hits"])
        self.assertEqual(2, features._stats["misses"])
    ...
    def test_untrusted_embedding_runs_body_without_reusing_or_writing(self):
        with self._build():
            first = _plate()
            with mock.patch.object(features, "_reuse_trusted", False):
                self.assertFalse(features.install(trusted_worker=True))
                second = _plate()
                third = _plate()
                self.assertFalse(features._reuse_trusted)
        self.assertEqual(_digest(first), _digest(second))
        self.assertEqual(_digest(first), _digest(third))
        self.assertEqual(0, features._stats["hits"])
        self.assertEqual(3, features._stats["misses"])

    def test_initial_embedding_patch_is_not_a_reuse_witness(self):
        program = Path(self.temp.name) / "embedding.py"
        program.write_text("""\
from pathlib import Path
from cadgen import features, feature, build123d as bd
...
with ExecutionHashes() as hashes, building(Path(__file__), 'embedding'):
    ...
    first = geometry()
    WIDTH = 3
    assert not features.install(trusted_worker=True)
    second = geometry()
assert not features._reuse_trusted
assert features._stats['hits'] == 0
assert abs(first.volume - 2) < 1e-8
assert abs(second.volume - 3) < 1e-8
assert not any(read_entry('op', key).get('featureScheme') for key, _ in iter_entries('op'))
""", encoding="utf-8")
        completed = subprocess.run([sys.executable, str(program)], capture_output=True,
                                   text=True, timeout=20,
                                   env={**os.environ, "PYTHONPATH": str(Path(features.__file__).parents[1])})
    ...
    def test_argument_and_keyword_default_changes_invalidate(self):
        ...
        self.assertEqual(3, features._stats["misses"])
        self.assertEqual(1, features._stats["hits"])
    ...
    def test_missing_or_corrupt_required_object_recomputes_and_repairs(self):
        ...
        with self._build():
            ...
            entries = [entry for entry in entries if entry.get("featureScheme")]
    ...
    def test_mutable_inputs_globals_and_dynamic_io_execute_normally(self):
        ...
        self.assertEqual(6, features._stats["declined"])
    ...
    def test_external_builders_and_locations_keep_their_side_effects(self):
        with self._build():
            ...
            for _ in range(2):
                ...
                self.assertEqual(1, features._stats["misses"])
            ...
            with bd.BuildSketch(bd.Plane.XY.offset(10)):
                with self.assertRaises(RuntimeError):
                    _builder_effect()
        self.assertGreaterEqual(features._stats["declined"], 4)
    ...
    def test_replaced_export_or_protocol_is_never_blessed_by_reinstall(self):
        ...
        with self._build():
            ...
            with mock.patch.object(build123d.Shape, "__bool__", lambda self: True):
                self.assertFalse(features.install())
                _plate()
            again = _plate()
        self.assertNotEqual(_digest(first), _digest(changed))
        self.assertEqual(_digest(first), _digest(again))
        self.assertEqual(2, features._stats["declined"])
        self.assertEqual(1, features._stats["hits"])
    ...
    def test_implicit_protocol_in_place_code_patch_declines(self):
        ...
        self.assertEqual(1, features._stats["declined"])
    ...
    def test_metaclass_property_change_declines(self):
        with mock.patch.object(type(bd.Plane), "XY", property(lambda cls: bd.Plane((4, 0, 0)))):
            self.assertFalse(features._runtime_eligible())
    ...
    def test_primitive_subclass_keeps_its_constructor_attributes(self):
        ...
        self.assertEqual(2, features._stats["unstorable"])
    ...
    def test_identity_sensitive_arguments_and_nonfinite_values_do_not_alias(self):
        ...
        self.assertEqual(4, features._stats["declined"])
    ...
    def test_replaced_bounding_box_intermediary_declines(self):
        ...
        measured = feature(namespace["measured"])
        with self._build():
            first = measured()
    ...
    def test_mutable_native_default_is_compared_by_value(self):
        ...
        axis = next(value for value, _key in features._trusted[-1] if type(value) is bd.Axis)
        ...
        self.assertTrue(features._runtime_eligible())
