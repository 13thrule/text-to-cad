...
- Floating toolbar: `Select` copies STEP topology references, `Pan` drags the camera, `Measure` picks measurement points, `Draw` opens annotation tools, `Play`/`Pause` appears when the model has animation clips, and `Copy screenshot` puts a viewport capture on the clipboard — screenshots are clipboard-only; nothing downloads. DXF drawings get their own 2D/3D view pill beside the toolbar. The navbar's `Fullscreen` button hands the model the whole window and auto-rotates it; the toolbar there keeps playback, capture and an `Exit fullscreen` button, and auto-hides until the pointer reaches it.
