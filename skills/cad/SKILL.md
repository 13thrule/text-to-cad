...
**Normal and Render snapshots.** With no `--render`, snapshots use deterministic light CAD lighting, an orthographic isometric camera, normal shaded-with-edges display, and no grid or axis guides. `--render studio-light|studio-dark` opts into the viewer's Render scene: perspective, shaded authored materials, no edges or guides, and high scene quality. `--render '{}'` uses the Light studio without pinning a studio in the reusable envelope. It also accepts the Render panel's exported JSON envelope or a path to it; an omitted `studio` follows the app appearance when pasted into the viewer and resolves Light in the CLI. `render.appearance` and the former studio ids were retired. `--camera` and `--display` are explicit overrides and win over the Render envelope. Projection and the positive `orthographicHalfHeight` view scale belong in camera JSON; guide, mode, edge and part-colour choices belong in display JSON. `--theme` was retired; use `--render`.
